 ...
 * Carga y registra automáticamente todos los event handlers desde el directorio de eventos.
 *
 * @class EventManager
 *
 * @example
    ...
    async loadEvents(): Promise<void> {
        ...
        for (const file of eventFiles) {
            ...
            if (event?.name && event.execute) {
                ...
                logger.debug('EventManager', `Registrado: ${event.name}${event.once ? ' (once)' : ''}`);
            }
        }
